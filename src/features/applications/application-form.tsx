  ...
  return (
    <form action={formAction} className="space-y-5 rounded-lg border border-[var(--line)] bg-white p-5 shadow-sm">
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Company" name="company" placeholder="Company name" error={state.fieldErrors?.company?.[0]} />
        <Field
          label="Role title"
          name="roleTitle"
        ...
        />
        <Field label="Location" name="location" placeholder="Copenhagen, Denmark" />
        <Field label="Deadline" name="deadline" type="date" error={state.fieldErrors?.deadline?.[0]} />
        <Field label="Job URL" name="jobUrl" type="url" placeholder="https://..." error={state.fieldErrors?.jobUrl?.[0]} />
        <Field label="Contact" name="contactName" placeholder="Recruiter or hiring manager" />
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <SelectField label="Status" name="status" values={APPLICATION_STATUSES} />
        <SelectField label="Work mode" name="workMode" values={workModes} />
      </div>
